...
const JobCard = ({ data }) => {
  ...
  let isSavedJob =
    isSignedIn && likedJobs?.some((obj) => obj.job._id === data._id);
      ...
      <div className="flex justify-between items-center">
        <div className="flex flex-row gap-x-3">
          <img
            ...
            src={data?.company.logo ?? "https://placehold.co/600x400/000000/FFF"}
            width="45"
            height="45"
          />
          <div className="text-stone-800 dark:text-stone-200">
            ...
            <div className="text-sm">{data?.company?.companyName}</div>
          </div>
        </div>

        <SaveButton onClick={onSave} isSaved={isSavedJob} className="w-8 h-8" />
      </div>
      ...
      {data.skillsets?.length ? (
        <div className="flex flex-wrap items-center gap-x-3 mb-3">
          {data.skillsets?.length > 0 &&
            data.skillsets.map((skill) => <Tags key={skill} name={skill} />)}
        </div>
      ) : null}
